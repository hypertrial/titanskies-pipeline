...
WAREHOUSE_SCHEMA_VERSION = "0.7.0"
...
def _prepare_v07_boundary(conn: duckdb.DuckDBPyConnection) -> None:
    """Reject every older populated warehouse; v0.7 intentionally has no migration."""
    if _table_exists(conn, TITANSKIES_OPS_SCHEMA, "warehouse_metadata"):
        ...
        if shared:
            raise RuntimeError(
                "TitanSkies warehouse schema 0.7.0 requires a clean rebuild; "
    ...
    if populated:
        raise RuntimeError(
            "TitanSkies warehouse schema 0.7.0 requires a clean rebuild; this "
            "database contains pre-0.7 rows. Back it up and create a new warehouse."
        )
