...
TEMPO_NO2_STD_DISCOVERY_SELECTION = AssetSelection.assets(
    asset_key(SOURCE_TEMPO, SCOPE_NO2_STD, "raw", "granule_inventory"),
)

TEMPO_NO2_STD_PROCESSING_SELECTION = AssetSelection.assets(
    asset_key(SOURCE_TEMPO, SCOPE_NO2_STD, "raw", "region_hour_aggregates"),
)
...
tempo_no2_std_granule_discovery = define_asset_job(
    TEMPO_NO2_STD_SCOPE.discovery_job_name,
    selection=TEMPO_NO2_STD_DISCOVERY_SELECTION,
    executor_def=_ANALYTICS_BUILD_EXECUTOR,
    config=tempo_no2_std_granule_discovery_run_config(),
    tags=_TEMPO_NO2_STD_TAGS,
)

tempo_no2_std_hourly_ingest = define_asset_job(
    TEMPO_NO2_STD_SCOPE.ingest_job_name,
    selection=TEMPO_NO2_STD_PROCESSING_SELECTION,
    executor_def=_ANALYTICS_BUILD_EXECUTOR,
    config=tempo_no2_std_hourly_ingest_run_config(),
    tags=_TEMPO_NO2_STD_TAGS,
)

tempo_no2_std_dbt_build = define_asset_job(
    TEMPO_NO2_STD_SCOPE.dbt_job_name,
    selection=TEMPO_NO2_STD_DBT_SELECTION,
    executor_def=_ANALYTICS_BUILD_EXECUTOR,
    config=tempo_no2_std_dbt_build_run_config(),
    tags=_TEMPO_NO2_STD_TAGS,
)

tempo_no2_std_full_pipeline = define_asset_job(
    TEMPO_NO2_STD_SCOPE.full_job_name,
    selection=TEMPO_NO2_STD_FULL_PIPELINE_SELECTION,
    executor_def=_ANALYTICS_BUILD_EXECUTOR,
    config=tempo_no2_std_full_pipeline_run_config(),
    tags=_TEMPO_NO2_STD_TAGS,
)
