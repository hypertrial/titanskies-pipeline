...
def test_dbt_translator_does_not_override_model_dependencies():
    assert "get_asset_spec" not in TitanSkiesDagsterDbtTranslator.__dict__


def test_dbt_translator_enables_source_visibility_settings():
    settings = TitanSkiesDagsterDbtTranslator().settings
...
def test_dbt_translator_group_name():
    translator = TitanSkiesDagsterDbtTranslator()
